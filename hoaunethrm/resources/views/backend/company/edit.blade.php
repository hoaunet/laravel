...
<section class="content-header">
 ...
 <section class="content"> 
  {!! Form::open(array('url' => 'company/edit/'. $company->mc_id,'id'=>'frmEdit', 'method' => 'post')) !!}
    ...
    <div class="box-body">
     <div class="control-group">
      ...
      <div class="row" id="field_title">
        ...
        <div class="controls col-sm-6"><input class="form-control" type="text" id="mc_title" name="mc_title" value="{{$company->mc_title}}"></div>      
  ...
  <div class="controls col-sm-6">
    <textarea class="form-control" type="textarea" rows="4" id="mc_description" name="mc_description" >{{$company->mc_description}}</textarea>
  </div>
  ...
  <div class="row" id="field_address">
    ...
    <div class="controls col-sm-6">
      <textarea class="form-control" type="textarea" rows="4" id="mc_address" name="mc_address" validation="none" value="{{$company->mc_address}}"></textarea>
    </div>
    <div class="controls col-sm-3">
    ...
    <div class="row" id="field_type">
      ...
      <div class="controls col-sm-6">
        <select type="select-one" class="form-control" id="mc_type" name="mc_type">
          ...
          <option value="{{ $key }}" @if($key==$company->mc_type) selected="" @endif>{{ $company_type }} </option> 
          @endforeach
          </select>
      </div>
      <div class="controls col-sm-3">
    ...
    <div class="row" id="field_country">
      ...
      <div class="controls col-sm-6">
        <select type="select-one" class="form-control select2Field select2-offscreen" id="mc_country" name="mc_country" value="{{old('mc_country')}}">
          ...
          <option value="{{$country->country_code}}" @if($country->country_code==$company->mc_country) selected="" @endif>{{$country->country_name}} </option> 
          @endforeach
        </select>
      </div>
      <div class="controls col-sm-3">
    ...
    <div class="row" id="field_timezone">
      ...
      <div class="controls col-sm-6">
        <select type="select-one" class="form-control select2Field select2-offscreen" id="mc_timezone" name="mc_timezone" value="{{old('mc_timezone')}}">
          ...
          <option value="{{$time_zone->mtz_name}}" @if($key==$company->mc_timezone) selected="" @endif>{{$time_zone->mtz_details}} </option> 
          @endforeach
          </select>
      </div>
      <div class="controls col-sm-3">
    ...
    <div class="row" id="field_parent">
      ...
      <div class="controls col-sm-6">
        <select type="select-one" class="form-control" id="mc_parent" name="mc_parent">
         @foreach($company_types as $key=>$company_type)
          <option value="{{ $key }}" @if($key==$company->mc_parent) selected="" @endif>{{ $company_type }} </option> 
          @endforeach  
        </select>
      </div>
      <div class="controls col-sm-3">
    ...
    <div class="row" id="field_heads">
      ...
      <div class="controls col-sm-6">
        <input class="form-control" type="text" id="mc_heads" name="mc_heads" value="{{$company->mc_heads}}">
